...
from pagefolio.app import (
    build_keysym_from_event,
    find_duplicate_binding,
    keysym_to_display,
    merge_shortcuts,
    shift_variant_keysym,
)
...
class TestBuildKeysymFromEvent:
    """build_keysym_from_event の検証（V171-UIUX-01・D-02・合成データ・Tk 不要）。"""

    def test_no_modifier(self):
        assert build_keysym_from_event(0, "F5") == "<F5>"

    def test_control_modifier(self):
        assert build_keysym_from_event(0x4, "o") == "<Control-o>"

    def test_shift_modifier(self):
        assert build_keysym_from_event(0x1, "Delete") == "<Shift-Delete>"

    def test_control_alt_shift_join_order(self):
        state = 0x4 | 0x20000 | 0x1
        assert build_keysym_from_event(state, "x") == "<Control-Alt-Shift-x>"


class TestFindDuplicateBinding:
    """find_duplicate_binding の検証（V171-UIUX-01・D-04・合成データ・Tk 不要）。"""

    SHORTCUTS = {"open_file": "<Control-o>", "save_file": "<Control-s>"}

    def test_detects_conflict_with_other_command(self):
        assert (
            find_duplicate_binding(self.SHORTCUTS, "undo", "<Control-o>") == "open_file"
        )

    def test_self_conflict_returns_none(self):
        assert (
            find_duplicate_binding(self.SHORTCUTS, "open_file", "<Control-o>") is None
        )

    def test_empty_keysym_returns_none(self):
        assert find_duplicate_binding(self.SHORTCUTS, "undo", "") is None

    def test_no_conflict_returns_none(self):
        assert find_duplicate_binding(self.SHORTCUTS, "undo", "<Control-z>") is None


class TestKeysymToDisplay:
    """keysym_to_display の検証（V171-UIUX-01・D-07・合成データ・Tk 不要）。"""

    def test_control_o(self):
        assert keysym_to_display("<Control-o>") == "Ctrl+O"

    def test_shift_delete(self):
        assert keysym_to_display("<Shift-Delete>") == "Shift+Delete"

    def test_f5(self):
        assert keysym_to_display("<F5>") == "F5"

    def test_empty_string(self):
        assert keysym_to_display("") == ""
