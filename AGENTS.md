# AGENTS.md — PageFolio AI 開発指示書

このファイルは Codex (AI) がこのプロジェクトを編集・拡張する際に参照する指示書です。
...
この出力は ChatGPT に貼り付けて Notion を更新するために使用する。
**書式は `session-handoff` スキル（`.agents/skills/session-handoff/SKILL.md`）を参照すること。**
