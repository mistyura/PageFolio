...
4. Windows 以外（`os.startfile` が存在しない環境）は未対応の情報通知に留める
...
4. `OCRDialog._start_worker_thread()` が生 `threading.Thread` デーモンワーカーを起動し、`OCRProvider.ocr_image(b64_png, prompt)` を実行する。`ocr.py` の `run_parallel` / `run_with_bounded_buffer` ヘルパー内部では `ThreadPoolExecutor` により複数ページを並列 OCR する
...
| 抽象 | ファイル | 説明 |
|------|----------|------|
| `PDFEditorApp` | `pagefolio/app.py` | 8 つの Mixin を多重継承するルートクラス。全状態を保持し、キーバインドを設定する |
...
| `OCRMixin` | `pagefolio/ocr.py` | プロバイダ生成（`build_provider`）・並列 OCR 実行・リトライ制御・ボタン状態管理を実装する |
| `PrintOpsMixin` | `pagefolio/print_ops.py` | 印刷（既定 PDF ハンドラ送信・`write_print_tempfile`）を実装する |
| `OCRProvider` | `pagefolio/ocr_providers.py` | `ocr_image(b64_png, prompt)` / `ocr_image_ex` / `list_models()` を持つ抽象基底クラス。`complete_text_ex` / `supports_text_prompt` で全ページ統合サマリを生成する |
...
| `OCRDialog` | `pagefolio/ocr_dialog.py` | 複数ページ OCR の進捗表示・結果ビューア・エクスポート UI（`_run_gen` 世代ガードで競合防止・Markdown 整形描画） |
...
│   ├── dialogs/             # ダイアログパッケージ（About / Settings / Plugin / Merge / LLMConfig / ExportImages / Password）
...
- `undo_store.py` / `pagination.py` / `md_render.py` を Tk・fitz 非依存の純関数層として切り出すことで、GUI を起動せずロジックだけを単体テストできるようにしている
...
- GUI の OCR フロー（`OCRMixin._start_ocr` → `OCRDialog`）は生 `threading.Thread` デーモンワーカーで実行する。`ocr.py` 側のヘルパー `run_parallel` / `run_with_bounded_buffer` は内部で `ThreadPoolExecutor` を用いてプロバイダ非依存の並列 OCR を行う。`fitz.Document` はスレッド間で共有せず、メインスレッドでレンダリングした base64 PNG のみワーカースレッドへ渡す
