...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| PyMuPDF | 1.28.0 | PDF 読み書き・レンダリング（`import fitz`） |
| Pillow | 12.3.0 | 画像変換・プレビュー表示 |
| tkinterdnd2 | 0.6.2 | ファイルのドラッグ＆ドロップ対応 |
| pyinstaller | 6.21.0 | Windows 実行ファイルのビルド（配布用、通常実行には不要） |
| pytest | 9.1.1 | テストランナー（開発用） |
| pytest-cov | 7.1.0 | カバレッジ計測（開発用） |
| ruff | 0.15.20 | リント・フォーマット（開発用） |
...
プロバイダ→環境変数の対応は `pagefolio/ocr_providers/registry.py` の `PROVIDER_ENV_KEYS` が唯一の定義元です。
