...
`pagefolio/ocr_providers/base.py` に定義（`pagefolio/ocr_providers/__init__.py` から再エクスポート）。すべてのプロバイダはこのクラスを継承します。
...
| 属性 | 説明 |
|------|------|
| `default_concurrency` | `run_parallel` がデフォルトで使用する並列度 |
...
| 引数 | 説明 |
|------|------|
| `lang` | Tesseract に渡す要求言語コード（`__init__` 時に `available_langs`（検出済み利用可能言語集合）との照合で `_resolve_lang` が段階的縮退を行い `self.effective_lang` を確定する） |
...
`pagefolio/ocr.py` の `run_parallel` と、`pagefolio/ocr_pipeline.py` の producer-consumer パイプライン API の 2 種類があります。
...
### `pagefolio/ocr_pipeline.py`（producer-consumer 純ロジック層）

`ocr_dialog.py` のメインスレッド `after()` 連鎖による producer（`fitz.get_pixmap()` はメインスレッド専用のため）と、複数の consumer ワーカースレッドとの間で安全にページをやり取りするための Tk/fitz 非依存のユーティリティ群です。全ページの base64 を同時にメモリへ保持せず、上限付きキューを介して OCR を実行できます。

#### `PipelineState`

producer-consumer 実行中の共有状態（完了件数・連続失敗数・fatal 情報・残ワーカー数）を `threading.Lock` で保護して管理するクラスです。
...
| メソッド | 説明 |
|---------|------|
| `record_success()` | ページ成功を記録し、連続失敗カウンタをリセット |
| `record_retryable_failure(msg, breaker_threshold=3)` | リトライ上限到達を記録。連続失敗数が閾値に達すると `fatal_kind="circuit_breaker"` を設定（戻り値: サーキットブレーカー発動有無の bool） |
| `record_fatal(msg, kind)` | 致命的エラー（`"connection"` / `"timeout"`）を記録（最初の1回のみ反映） |
| `record_page_error(msg)` | 非致命的なページ単位エラー（`RuntimeError` 等）を記録 |
| `is_fatal()` | fatal 確定済みかどうかを返す |
| `decrement_worker()` | ワーカー終了を1本減算し `(is_last, fatal_msg, fatal_kind)` を返す |

#### `try_enqueue(q, item) -> bool`

`item` を `q` へ非ブロッキングで enqueue します。成功時 `True`・`queue.Full` 時 `False` を返します（例外は送出しません）。

#### `send_sentinels(q, count) -> int`

終端シグナル（`None`）を最大 `count` 個非ブロッキングで送出し、実際に送れた本数を返します（部分送出可・呼び出し元が残り本数を再試行）。

#### `consume_one(provider, item, prompt, state, cancel_check=None, breaker_threshold=3, on_success=None, on_page_error=None, on_fatal=None, on_retry_wait=None)`

キューから取り出した1アイテム `(page_idx, b64_png)` を消費します。`provider.ocr_image_ex()` を呼び出し、`OCRRetryableError` はリトライ（`pagefolio.ocr` の `MAX_RETRIES`・`clamp_retry_after`・`interruptible_sleep` を使用）、`ConnectionError`/`TimeoutError` は致命的エラーとして `state` に記録し、対応するコールバックを呼びます。戻り値はなく、結果は `state` の更新とコールバック経由でのみ伝達されます。
...
`OCRRetryableError`（HTTP 429 / 5xx）が発生した場合、`run_parallel`（`pagefolio/ocr.py`）は自動的にリトライします。`ocr_pipeline.py` の `consume_one` も同じリトライロジックを使用します。
...
| プロバイダ設定 UI | `pagefolio/dialogs/llm_config/`（パッケージ）— `LLMConfigDialog`（`__init__.py`） |
