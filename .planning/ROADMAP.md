...
- [x] **Phase 3: 品質保証・リリースゲート** - Tkinter 実行環境を修復してGUIテスト含む全テストを完走させ、保存トースト再試行時の上書き確認再表示とhuman-verify/UATを正式実施してリリース判定を固める (completed 2026-08-11)
...
- [x] 02-01-PLAN.md — 能力マトリクス確定 + カタログ基盤 + OpenAI プロバイダ + build_provider の縦スライス（tracer・V190-CAT-01/02・V190-OAI-02/11/12/13）
...
- [x] 02-03-PLAN.md — LLM 設定 UI の OpenAI セクション・セッション限定キー・モデル一覧取得（確認済み優先の並び替え）とプロバイダ一覧の catalog 化（V190-CAT-01・V190-OAI-01/02/03）
...
**Goal**: Python 3.14.6 環境の Tkinter 実行問題が解消されて GUI テストを含む全テストが完走し、保存トースト再試行時に上書き確認・保存先選択が再表示されず前回確定した対象へ黙って再保存され、実機目視による human-verify/UAT が正式に実施・記録されてリリース判定ができる。
...
**Success Criteria** (what must be TRUE):
  ...
  2. 保存トーストの再試行を実行すると、上書き確認・保存先選択が再表示されず、前回確定した対象へ黙って再保存される（V190-QA-02・D-09/D-12 により文言訂正）
...
- [x] 03-01-PLAN.md — 保存トースト再試行の確認スキップ（保存 3 経路を確認層＋実保存層へ分離・D-12 文言訂正）
- [x] 03-02-PLAN.md — テスト実行環境の 2 症状切り分けとリリースゲート合格条件の文書化
- [x] 03-03-PLAN.md — 遡及 human-verify/UAT の対象確定・実機確認 3 分割・結果記録
- [x] 03-04-PLAN.md — リリース版数文書の 3 点同期（APP_VERSION / README バッジ / 開発履歴.md）
...
| 3. API 整理と回帰テスト | v1.3.0 | 4/4 | Complete    | 2026-06-03 |
