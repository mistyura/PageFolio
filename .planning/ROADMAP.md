...
- [ ] **Phase 1: APIキー入力欄（LLM設定への一元化）** - クラウド3種のキー入力欄追加・入力値優先の解決順反転・OCRDialog 側入力欄の撤去・RunPod のセッションキー対応 + その回帰テスト
- [ ] **Phase 2: OCR 磨き込み（レビュー残の現行照合と二重実装解消）** - L-6 小物一括解消・tesseract_lang 尊重・プラグイン registry 堅牢化・producer-consumer 一本化（L-1 は独立プランへ隔離）
- [ ] **Phase 3: ページ操作磨き込み + v1.5.0 回帰テスト** - 画像透かし対応・黒塗り/モザイク・回転/トリミングの棚卸し改善・v1.5.0 新機能の回帰テスト整備
...
- [ ] 01-01-PLAN.md — キー解決順の反転（入力値→環境変数）+ 解決系テスト（claude/gemini 書き換え・RunPod 新設）+ 新規/更新 LANG 文言（Wave 1）

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 01-02-PLAN.md — LLMConfigDialog に3プロバイダの APIキー入力欄・トグル・注記追加 + 2経路配線（SettingsDialog 側）+ ライブ値モデル取得 + 非流入/格納/スロット回帰テスト（Wave 2）

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 01-03-PLAN.md — OCRDialog 旧セッションキー UI/ヘルパー撤去 + 実行前ゲート _check_cloud_api_key 置換 + _open_llm_config 配線 + 後継テスト（Wave 3）
...
> **リスク注記:** V171-OCR-04（L-1 一本化）は `ocr.py`/`ocr_dialog.py` を横断する高リスク項目のため、計画時に**フェーズ内の独立プランへ隔離**し、他の OCR 磨き込みプランと分離して実行・検証する。
...
| 2. OCR 磨き込み（レビュー残の現行照合と二重実装解消） | v1.7.1 | 0/? | Not started | - |
