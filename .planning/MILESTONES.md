...
**Note:** GSD フェーズ外のブランチ作業。実装コミット `60c3acc` / ビルド `c4fbc2d`、`main` へ PR #26 でマージ（マージコミット `ae16c22`）。注釈付きタグ `v1.6.2-1`・GitHub Release を Latest 公開（2026-06-30、`PageFolio-v1.6.2-win64.zip` + `.sha256` 添付）。リリースタグは immutable release のタグ名再利用ブロック回避で `-1` サフィックスを付与（当初 `v1.6.2` で notes-only 公開→アセット追加不可のため作り直し。`v1.6.0-3` と同様の衝突回避パターン）。本エントリは出荷後に遡及追記。
