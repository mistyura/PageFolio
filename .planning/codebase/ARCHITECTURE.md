...
- **Blob storage:** Undo stack holds Blob references (FileBlob/MemBlob). Lifecycle: capture on operation → push to stack → pop on undo/redo or evict → release temp file. Capture MUST go through `_capture_page_blob(page_i)` and restore MUST go through `self._blob_bytes(data)` (raw `bytes` accepted for backward compatibility). Direct `append()`/`clear()` to `_undo_stack` bypasses cleanup and leaks files.
