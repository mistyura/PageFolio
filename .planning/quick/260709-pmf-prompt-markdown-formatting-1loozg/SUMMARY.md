...
双方向連動化）→ `8ef30d5`（Markdown 整形表示のプリセット一本化）→
`288002e`（OCR ダイアログ右ペインの縦スクロール対応）— いずれも push 済み
...
### コミット `288002e`: OCR ダイアログ右ペインの縦スクロール対応

ユーザー要望「右ペイン（コマンドボタン）にも縦スクロールバーをつけて。足りない場合は
アイコンのみの短縮形も調査」を受けて実装・調査した（v1.7.3 の申し送り解消）。

- セクション群（「▶ 実行」「📋 結果」）を Canvas + Scrollbar 構成へ変更
  （`ui_builder._build_tools_scrollable` と同型: inner Frame を `create_window` で
  載せ scrollregion / 幅同期・マウスホイール再帰バインド・破棄後 TclError ガード）。
  Canvas 幅は内容の要求幅（約 229px）へ追随し従来のペイン幅を維持。
- 「✕ 閉じる」はスクロール領域外（右ペイン最下部固定）で常時可視。
- **調査結果（Xvfb 実測）**: 既定フォント 10・既定サイズ 1150x680 はスクロール不要
  （内容 310px < 可視 349px）。最小 960x620 ではフォント 10/16 ともスクロールで
  最下部ボタンまで到達可能・閉じる常時可視。幅は最長ラベル基準で問題ないため
  **アイコンのみの短縮ラベル化は不要と判断**（高さ不足はスクロールで吸収）。
...
- 右ペインのマウスホイールバインドは Windows の `<MouseWheel>` のみ（メイン画面と
  同仕様・Linux の Button-4/5 は対象外）。ホイール動作は Windows 実機で要確認。
