...
**Shipped:** 2026-08-11
**Phases:** 3 | **Plans:** 15 | **Tasks:** 48 | **Sessions:** 複数（2026-08-10 計画、08-11 実装〜クローズ）

### What Was Built
- **保存・編集・設定の安全性是正（Phase 1・7プラン）**: 保存3経路＋縮小保存の2分岐に `encryption=PDF_ENCRYPT_KEEP` を構造的に既定化し、`derive_pdf_has_password` 純関数で `pdf_has_password` の導出を1箇所へ統一。`build_provider` の `off` を専用例外 `OCRDisabledError` で拒否して OCR OFF を4経路で同一の意味へ。複数ファイル挿入のロールバック・ページ複製の Undo 後置化・`_undo`/`_redo` 復元失敗時の `_push_evicting` による state 保全。外部プロンプトファイル書き込みを Apply の1経路へ一本化。
- **Undo/Redo の構造的堅牢化（Phase 1・01-06/01-07 のギャップ是正2回）**: 部分失敗→再試行後に逆デルタが「再試行分のみに縮小」して確定するサイレントなページ破損を7 opで解消（`_pending_inverse` 蓄積方式）。`page_edit` の2段階 mutation を `delete_page`→`insert_pdf` から `insert_pdf`→`delete_page` へ反転し、doc がページ内容を失っている瞬間を構造的に排除（option-b）。
- **OCR プロバイダ基盤整理 + OpenAI 追加（Phase 2・4プラン）**: `ocr_providers/catalog.py` を新設しプロバイダメタデータを単一情報源化（6参照面すべて移行完走）。`registry.py` の標準ライブラリのみ依存の独立性制約は維持。OpenAI(ChatGPT) を `urllib` 直叩き・新規 pip 依存ゼロで実装し、セッション限定キー・実 API モデル一覧＋静的フォールバック・送信先/コスト確認・バッチ OCR・明示設定型フォールバック・detail / reasoning effort / organization / project を既存5プロバイダと同等の安全境界で組み込み。
- **品質保証・リリースゲート（Phase 3・4プラン）**: テスト環境の2症状を10回連続実行で非再現と一次データで反証しコード変更ゼロでクローズ（累計17回連続グリーン）。リリースゲートを単一プロセス `pytest -q` 完走に確定。保存トースト再試行の確認再表示を確認層/実保存層の分離で解消。遡及 human-verify/UAT を正式実施（実施13 / 未実施2 / 対象外1）。

### What Worked
- **「調べる前に直さない」の徹底（V190-D-QA01）**: テスト環境の2症状に対し、予防的な `TCL_LIBRARY`/`TK_LIBRARY` ハードコード（リサーチの PITFALLS が推奨していた）を入れず、まず現行 HEAD で10回連続の再現試行を行った。結果は 0/10 で非再現。コード変更ゼロで閉じられただけでなく、次マイルストーンが同じ地面を掛け直さないための一次データが残った。リサーチ間で意見が割れた（STACK: 実機再現せず／PITFALLS: 早期対応推奨）ときに、推測ではなく実測で決着させた判断が効いた。
- **検証ギャップの2段階クロージャが実際にデータ破損を捕まえた**: Phase 1 の初回検証が gaps_found（4/5）で、01-06・01-07 の2回のギャップ是正プランを経て 5/5 passed へ到達。特に 01-06 が解消した「部分失敗→再試行後の逆デルタ縮小」はサイレントなデータ破損であり、通常の往復テストでは検出できない種類のバグだった。verifier を通さず「プラン完了＝フェーズ完了」としていたら出荷していた。
- **コードレビューが BLOCKER 級のデータ損失を1件捕まえた**: Phase 3 の 03-01（保存トースト再試行）で、パスのみ `functools.partial` で束縛して `self.doc` を都度参照する実装が「トースト表示中に別ファイルを開くと無関係な Document を確定パスへ無確認上書きする」経路を作っていた。`bound_doc` による Document 同一性ガードで構造的に排除し、回帰テスト6件で固定。v1.8.0 で確立した「コードレビュー即時修正パターン」が2マイルストーン連続で実害を防いだ。
- **3マイルストーン続いた「一旦 pass」運用の解消（V190-QA-03）**: v1.4.0/v1.6.0/v1.7.1 で計3回、human-verify をユーザー判断で未検証のまま pass にしてきた負債を、候補14項目を現行コード照合で「活き残り13項目」に確定したうえでユーザーの実機目視により正式消化した。未実施2項目（実 API 課金・`ANTHROPIC_API_KEY` 未設定）は理由付きで残し、推測で pass にしなかった。
- **APP_VERSION 更新漏れの再発が止まった**: v1.6.0/v1.7.1/v1.8.0 と3回連続で発生していた漏れに対し、v1.9.0 では 01-05/01-06/01-07 の各プランが「版数文書の更新は Phase 3 の V190-QA-03 へ委譲する」と明示的に判断を記録し、Phase 3 の 03-04 プランで APP_VERSION / README バッジ / 開発履歴.md の3点同期として実行した。フェーズ計画の中に版数同期の置き場所を作ったことが効いた。

### What Was Inefficient
- **マイルストーン監査を Phase 2 完了時点で実行してしまった**: `v1.9.0-MILESTONE-AUDIT.md` を 17:03 に作成した40分後（17:43）に Phase 3 が始まり、監査が指摘した唯一のブロッカー（Phase 3 未着手・V190-QA-01/02/03 未充足）が全て解消された。クローズ時に「監査が gaps_found のまま」という状態を人手で再評価する必要が生じた。監査は全フェーズ完了後に一度だけ実行するのが正しい。
- **リリースゲートの収集件数がドキュメントと実態でずれた**: Phase 3 の 03-02 が CLAUDE.md へ「現在 1398 件収集」と記録した後、同じ Phase 3 の後続プランがテストを追加して実測 1404 件になった。マイルストーンクローズ時に検出・修正。件数のような変動する実測値をドキュメントに直書きすると、書いた瞬間から陳腐化が始まる。
- **フェーズアーカイブが CLAUDE.md の参照リンクを壊した**: `milestone complete` が `.planning/phases/` を `milestones/v1.9.0-phases/` へ移動した結果、CLAUDE.md のリリースゲート節が指す `03-TEST-ENV-INVESTIGATION.md` のパスが無効化された。クローズ時に検出・修正したが、`.planning/phases/` を CLAUDE.md 等の恒久ドキュメントから直接参照すると毎回のクローズで壊れる。
- **技術的負債10件がマイルストーン内で消化されなかった**: `page_ops.py` 側の WR-03〜WR-06 は `file_ops.py` 側で 01-07 が既に解消したパターンの水平展開であり、同じセッション内なら安価に閉じられた可能性が高い。Phase 3 に小規模プランとして含めるか判断する機会があったが、繰り越しを選んだ。

### Patterns Established
- **「調べる前に直さない」ゲート**: 環境起因が疑われる症状は、修正コードを書く前に現行 HEAD で N 回の再現試行を行い、一次データ（実行コマンド・件数・クラッシュ有無を行単位で記録した表）を残す。非再現ならコード変更ゼロで「解消済み」を反証データ付きで確定させる。
- **版数同期の置き場所をフェーズ計画に作る**: APP_VERSION / README バッジ / 開発履歴.md の3点同期を最終フェーズの独立プランとして明示的に配置し、中間フェーズは「Phase N へ委譲」と記録して触らない。
- **mutation 順序の反転による構造的解消**: 2段階 mutation（削除→挿入）が中間失敗でデータを失う場合、Blob 保持による復元（option-a）より mutation 順序の反転（挿入→削除・option-b）の方が、失われる瞬間そのものを消せるため強い。

### Key Lessons
1. **マイルストーン監査は全フェーズ完了後に一度だけ実行する**。途中で走らせると「未着手フェーズ＝ギャップ」として計上され、クローズ時に人手での再評価コストが発生する。
2. **リサーチ間で意見が割れたら実測で決着させる**。v1.9.0 の Tkinter 問題は、推奨に従って予防的修正を入れていたら、存在しない問題への恒久的なハードコードが残るところだった。
3. **verifier のギャップ是正ループは「動いているように見えるがデータを壊す」バグを捕まえる装置として実際に機能する**。01-06 が捕まえた逆デルタ縮小は往復テストを通過する種類の破損だった。
4. **恒久ドキュメント（CLAUDE.md 等）から `.planning/phases/` を直接参照しない**。マイルストーンクローズのアーカイブで必ず壊れる。参照するならアーカイブ後の安定パスか、リンクではなく内容の要約を置く。
5. **実測値（テスト件数等）をドキュメントに直書きするなら、それを更新する責任者をどのプランが持つかを決めておく**。

### Cost Observations
- Model mix: model_policy provider `anthropic-fable`（high=fable-5 / medium=sonnet-5 / low=sonnet-5）・mode `yolo`
- Sessions: 複数（2026-08-10 計画、08-11 実装〜クローズ）
- Notable: 149 コミット・115 ファイル変更・約 30,700 行追加/2,780 行削除を約2日間（2026-08-10〜08-11）で完了。うち製品コード/テストは 27 ファイル・約 10,960 行追加/1,700 行削除で、残りは計画ドキュメント。フェーズ3本という小さい構成に対し Phase 1 が7プラン（うち2つがギャップ是正）と肥大化した。
...
| v1.9.0 | 複数 | 3 | 「調べる前に直さない」ゲートを確立（環境問題を実測10回でコード変更ゼロクローズ）。版数同期を最終フェーズの独立プラン化して APP_VERSION 漏れの4回目再発を阻止。verifier ギャップ是正2周でサイレントなデータ破損を捕捉。3マイルストーン続いた human-verify「一旦 pass」運用を正式消化 |
...
| v1.9.0 | 1404 passed | （未計測） | catalog.py / openai プロバイダを標準ライブラリ（urllib）のみで新設。新規 pip 依存ゼロを6マイルストーン連続で維持 |
...
8. クロスマイルストーン衝突バグ（ROADMAP.md/STATE.mdの番号のみマッチ）はツール呼び出しの都度再発しうるため、実行直後の`git diff`確認を毎回徹底する（v1.8.0で3回検知。v1.9.0 Phase 3 でも ROADMAP Progress 表のフェーズ番号あいまい一致による誤更新を検知・訂正）。
9. 環境起因が疑われる症状は修正を書く前に N 回の再現試行で一次データを取る。非再現ならコード変更ゼロで反証データ付きにクローズできる（v1.9.0・V190-D-QA01。予防的ハードコードを入れずに済んだ）。
10. バージョン番号の同期は「注意する」ではなく最終フェーズの独立プランという置き場所で解決する（v1.9.0 で 4 回目の再発を阻止。教訓5への構造的な答え）。
11. マイルストーン監査は全フェーズ完了後に一度だけ実行する。途中実行は未着手フェーズをギャップとして計上し、クローズ時の再評価コストを生む（v1.9.0）。
12. 恒久ドキュメント（CLAUDE.md 等）から `.planning/phases/` を直接参照しない。マイルストーンクローズのアーカイブでリンクが必ず壊れる（v1.9.0）。
