...
> 計画時に全項目解決済み: Q1 は `pagefolio/thumb_cache.py` 新規モジュールとして独立（05-01-PLAN.md Task 1 で確定）、Q2 は `_bind_shortcuts` のラムダを `should_suppress_for_focused_input` ガード付きへ再構成（`focus_get()` の `None` 防御込み、05-04-PLAN.md Task 2 で確定）。
