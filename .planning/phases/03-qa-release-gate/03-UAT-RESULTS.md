...
Task 2〜4 の checkpoint 承認後、各グループの結果を随時記録する（最終確定・サマリ集計は
Task 5 で行う）。`結果` に使う値は `pass` / `fail` / `未実施` の3種のみ（「一旦 pass」は
使わない）。グループ1（⑥⑦⑧⑬）は Task 2 承認済み、グループ2（⑨⑩⑪⑫⑭）は Task 3 承認済みで
いずれもユーザーによる実機目視で全項目 pass。グループ3（④⑤-Gemini①②）は Task 4 の
承認待ちのため `—` のプレースホルダのまま。
